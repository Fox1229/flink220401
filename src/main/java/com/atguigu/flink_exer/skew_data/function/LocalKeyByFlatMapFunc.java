package com.atguigu.flink_exer.skew_data.function;
...
public class LocalKeyByFlatMapFunc extends RichFlatMapFunction<Event, UserViewCountPerWindow> implements CheckpointedFunction {
    ...
    @Override
    public void flatMap(Event value, Collector<UserViewCountPerWindow> out) throws Exception {
        // 1、将新来的数据添加到 buffer 中,本地聚合
        Long count = localBuffer.getOrDefault(value.username, 0L);
        localBuffer.put(value.username, count + 1);

        // 2、如果到达设定的批次，则将 buffer 中的数据发送到下游
        if (currentSize.incrementAndGet() >= batchSize) {
            // 2.1 遍历 Buffer 中数据，发送到下游
            for (Map.Entry<String, Long> kv : localBuffer.entrySet()) {
                out.collect(
