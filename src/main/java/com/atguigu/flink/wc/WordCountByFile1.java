...
public class WordCountByFile1 {
    ...
    public static void main(String[] args) throws Exception {
        ...
        env
                ...
                .setParallelism(1) // readTextFile只有一个并行子任务，占用一个任务插槽
                .flatMap(new FlatMapData())
                .setParallelism(1) // flatMap只有一个并行子任务，占用一个任务插槽
                // keyBy不是算子，所以不能设置并行度
                .keyBy(new KeyByData())
                .reduce(new SumData())
                .setParallelism(1) // reduce只有一个并行子任务，占用一个任务插槽
                .print()
                .setParallelism(1); // print只有一个并行子任务，占用一个任务插槽
