 ...
 * 1. 实现处理时间滑动窗口。实现累加器
 */
public class KeyedProcessFunImplSlidingWindow {
    ...
    public static class MySlidingWindow extends KeyedProcessFunction<String, UserBehavior, ProductViewCountPerWindow> {
        ...
        @Override
        public void processElement(UserBehavior action, Context ctx, Collector<ProductViewCountPerWindow> out) throws Exception {

            // 获取数据的第一个窗口开始时间
            long firstStartTime = action.ts - action.ts % slidingStep;

            // 计算数据所属的所有窗口的开始时间
            ArrayList<Long> windowStartList = new ArrayList<>();
            // ts = 7s, windowSize = 10s, slidingSize = 5
            //
            for (long i = firstStartTime; i > action.ts - windowSize; i -= slidingStep) {
                windowStartList.add(i);
            }

            // 添加数据
            for (Long tmp : windowStartList) {
                if (!mapState.contains(tmp)) {
                    mapState.put(tmp, 1L);
                } else {
                    mapState.put(tmp, mapState.get(tmp) + 1L);
                }
            }

            // 注册定时器
            for (Long tmp : windowStartList) {
                ctx.timerService().registerEventTimeTimer(tmp + windowSize - 1L);
            }
        }
